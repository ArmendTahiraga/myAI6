...
export function AssistantMessage({
	...
	const rewrittenTexts = rewriteCitationsInParts(textPartIndexes.map((i) => (message.parts[i] as { text: string }).text));
	const rewrittenByIndex = new Map<number, string>(textPartIndexes.map((partIndex, j) => [partIndex, rewrittenTexts[j]]));
		...
		<div className="w-full">
			<div className="text-sm flex flex-col gap-4">
				{message.parts.map((part, i) => {
					const isPartStreaming = isStreaming && i === message.parts.length - 1;
					...
					if (part.type === "text") {
						...
						const hasIntermediateProcessingText =
							isLastText &&
							seenTool &&
							message.parts.some((p, idx) => idx < i && p.type === "text" && hasToolBefore.has(idx));
						return (
							<div key={`${message.id}-${i}`}>
								{isLastText && isAfterTool && !hasIntermediateProcessingText && (
									<ProcessingIndicator isStreaming={false} />
								)}
								{isLastText && hasContentBefore && <AssemblingIndicator isStreaming={isPartStreaming} />}
								{!isLastText && isAfterTool && <ProcessingIndicator isStreaming={isPartStreaming} />}
								<Response isAnimating={isPartStreaming}>{rewrittenByIndex.get(i) ?? part.text}</Response>
							</div>
						);
