...
import { ArrowUp, Download, FileText, PanelLeft, Plus, Square } from "lucide-react";
...
export default function Chat() {
	...
	// updateConversationData merges the new data with the old data making sure nothing is lost
	useEffect(() => {
		if (isClient && activeConvId) {
			updateConversationData(activeConvId, { messages, durations });
		}
	...
	return (
		<div className="flex h-screen font-sans dark:bg-black">
			...
			<main className="relative h-screen flex-1 min-w-0">
				<div
					className={`fixed top-0 right-0 z-50 pb-16 pointer-events-none ${sidebarOpen ? "left-0 md:left-64" : "left-0"}`}
				>
					<ChatHeader>
						<ChatHeaderBlock>
							<Button
						...
						<ChatHeaderBlock className="justify-end gap-2">
											...
											<div className="absolute right-0 top-10 z-50 w-80 max-h-64 overflow-y-auto rounded-md border bg-background p-3 shadow-lg">
												<div className="flex items-center justify-between mb-2">
													...
													<span className="text-[10px] text-muted-foreground">
														{cs.summarizedUpTo} messages summarized
													</span>
												</div>
							...
							<Button variant="ghost" size="icon" onClick={exportChat} aria-label="Export chat" className="h-8 w-8">
											...
											isCompacting={(() => {
												...
												for (const msg of messages) {
													for (const part of msg.parts) {
														const p = part as any;
														chars +=
															p.type === "text" ? (p.text?.length ?? 0) : JSON.stringify(p).length;
													}
												}
