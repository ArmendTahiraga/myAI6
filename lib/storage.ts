...
export function loadConversationData(id: string): ConversationData {
	const empty: ConversationData = { messages: [], durations: {} };
	if (typeof window === "undefined") return empty;
	try {
		const raw = localStorage.getItem(DATA_PREFIX + id);
		if (!raw) return empty;
		const parsed = JSON.parse(raw);
		return { ...parsed, messages: parsed.messages || [], durations: parsed.durations || {} };
	} catch {
		return empty;
	}
}

export function saveConversationData(id: string, data: ConversationData) {
	if (typeof window === "undefined") return;
...
/** Merge a partial update into stored conversation data, keeping all other fields */
...
export function saveCompactedSummary(id: string, summary: string, summarizedUpTo: number, signature: string) {
...
export function loadCompactedSummary(id: string): {
	...
	const data = loadConversationData(id);
	if (data.compactedSummary && data.summarizedUpTo !== undefined && data.compactedSignature) {
		return {
...
export function saveFeedback(conversationId: string, messageId: string, rating: "up" | "down" | null) {
	...
	if (rating) {
		...
		delete data.feedback[messageId];
	}
